$(function(){
	function validateData(mark_input = true)
	{
		...
		var has_error = error_card_number || error_cvv || error_name || error_month || error_year;
        return !has_error;
	}

	function validateCardNumber(mark_input = true)
	{
		var input_bandeira = $('#agerede_debit_card [name=agerede_cardnumber]');
	...
	{
		var input_cvv = $('#agerede_debit_card [name=agerede_cvv]');
	...
	{
		var input_name = $('#agerede_debit_card [name=agerede_name]');
	...
	{
		var input_month = $('#agerede_debit_card [name=agerede_month]');
	...
	{
		var input_year = $('#agerede_debit_card [name=agerede_year]');
    ...
    function getBandeira(cardNumber)
    {
        ...
        var regexElo = /^(627780|636368|438935|504175|451416|636297)([0-9]{10})$/;
	...
	$('#agerede_debit_card [name=agerede_cardnumber]').mask('0000 0000 0000 0099');
	$('#agerede_debit_card [name=agerede_cvv]').mask('0009');

	$('#agerede_debit_card [name=agerede_cardnumber]').keyup(function(){
		var bandeira = getBandeira($(this).val());

		if (bandeira) {
			document.querySelector('#agerede_debit_card .agerede_cardbanner').value=bandeira;
            document.querySelector('#agerede_debit_card .agerede_cardbanner').innerHTML='<img src="' + agerede.base_uri + 'modules/agerede/views/img/cardbanners/'+bandeira+'.png" />';
		} else {
	...
	$('#agerede_debit_card select').change(function(){
		if ($(this).val() > 0) {
	...
	$('#agerede_debit_card input, #agerede_debit_card select').change(function(e){
	...
	$('#agerede_debit_card').submit(function(){
